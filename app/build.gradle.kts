...
android {
    ...
    defaultConfig {
        ...
        // 二改：移除 ndk abiFilters —— 与 splits.abi 冲突，ABI 由 splits 控制
    }
    ...
    splits {
        abi {
            ...
            reset()
            // 二改：只构建 arm64-v8a（手机真机使用，加快 CI 构建）
            include("arm64-v8a")
    ...
    signingConfigs {
        // 二改：CI/本地固定签名。把 keystore.jks 放到项目根目录 signing/ 下，
        // 并通过环境变量 CI_KEYSTORE_PASSWORD 提供密码即可启用；
        // 文件不存在时回退到默认 debug 签名，不影响官方构建逻辑。
        create("ci") {
