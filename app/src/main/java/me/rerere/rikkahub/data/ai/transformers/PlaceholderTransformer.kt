...
object DefaultPlaceholderProvider : PlaceholderProvider {
    override val placeholders: Map<String, PlaceholderInfo> = buildPlaceholders {
        ...
        placeholder("persona", { Text(stringResource(R.string.placeholder_persona)) }) {
            it.settingsStore.settingsFlow.value.displaySetting.userPersona.ifBlank { "" }
        }
    }
...
object PlaceholderTransformer : InputMessageTransformer, KoinComponent {
    ...
    override suspend fun transform(
        ...
        // ST persona 行为: 用户人设描述非空时, 附加到系统提示词末尾
        val persona = settingsStore.settingsFlow.value.displaySetting.userPersona.trim()
        if (persona.isNotEmpty()) {
            val userName = settingsStore.settingsFlow.value.displaySetting.userNickname.ifBlank { "user" }
            val systemIndex = replaced.indexOfFirst { it.role == MessageRole.SYSTEM }
            if (systemIndex >= 0) {
                val systemMessage = replaced[systemIndex]
                return replaced.toMutableList().apply {
                    this[systemIndex] = systemMessage.copy(
                        parts = systemMessage.parts + UIMessagePart.Text("\n\n$userName's persona: $persona")
                    )
                }
            }
        }
