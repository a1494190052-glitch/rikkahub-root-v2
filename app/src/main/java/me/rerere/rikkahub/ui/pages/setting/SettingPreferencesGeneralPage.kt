...
import me.rerere.rikkahub.service.FloatingTaskService
    ...
    Scaffold(
        ...
        LazyColumn(
            ...
            item {
                CardGroup {
                    item(
                        headlineContent = { Text(stringResource(R.string.setting_page_floating_task_window_title)) },
                        supportingContent = { Text(stringResource(R.string.setting_page_floating_task_window_desc)) },
                        trailingContent = {
                            Switch(
                                checked = settings.floatingTaskWindowEnabled,
                                onCheckedChange = { enabled ->
                                    vm.updateSettings(settings.copy(floatingTaskWindowEnabled = enabled))
                                    if (enabled) {
                                        if (!android.provider.Settings.canDrawOverlays(context)) {
                                            ...
                                            runCatching {
                                                context.startForegroundService(
                                                    Intent(context, FloatingTaskService::class.java)
                                                )
                                            }
                                        }
                                    }
